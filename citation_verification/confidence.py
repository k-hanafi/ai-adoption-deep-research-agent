"""Extract sampled-token verification confidence from Responses API logprobs.

Ported from taxonomy two_pass_classifier.confidence with decision key
`verification` (1=verified, 0=hallucination).
"""
...
from citation_verification.config import (
    DECISION_KEY,
...
class BinaryConfidence:
    ...
    @property
    def log_probs_conf(self) -> float:
        """Alias used by Stage 3 product schema."""
        return self.sampled_probability
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            **asdict(self),
            "log_probs_conf": self.log_probs_conf,
        ...
        for content in item.get("content") or []:
            if content.get("type") == "output_text":
                if not content.get("logprobs"):
                    raise LogprobExtractionError(
                        "output_text has no logprobs; judge must request them"
                    )
                return content
...
def _verify_reconstruction(text: str, logprobs: list[dict[str, Any]]) -> None:
...
def _find_decision_entry(
    ...
    for index, entry in enumerate(logprobs):
        ...
        position += width
    raise LogprobExtractionError(
        f"no logprob token covers the {DECISION_KEY} value"
    )
